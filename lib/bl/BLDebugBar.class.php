...
add_filter( 'debug_bar_panels', 'bl_load_debug_bar' );
function bl_load_debug_bar($panels) {
	if (!class_exists('BLDebugBar') && class_exists('Debug_Bar_Panel')) {
		class BLDebugBar extends Debug_Bar_Panel {

			private static $debug_log = array();
			
			function init() {
                                // Title to display in left column of debug bar
				$this->title( __('bl-plugin-template-debug-bar', BL_TEXTDOMAIN) );
                                
                                // Custom styling for the debug bar output
				wp_enqueue_style( 'bl-debug-bar-css', BL_PLUGIN_URL . 'css/debug-bar.css' );
                                
                                // Action hook called when new dbug info is submitted
                                add_action( 'bl_debug', array( &$this, 'logDebug' ), 10, 3 );
